...
def restart_game():
    ...
    player.isBlockedLeft = False
    player.isBlockedRight = False
    player.isBlockedUp = False
    player.isBlockedDown = False
...
class Player(pygame.sprite.Sprite):
    ...
    def check_collision(self,obj):
        self.distance = sqrt((pow((self.rect.x + 48) - (obj.rect.x + 96),2))+(pow((self.rect.y + 48) - (obj.rect.y + 96),2))) 
        if self.distance < 50:
            if self.isWalkingDown == True:
                self.yDownSpeed = 0
...
while True:
    ...
    player.check_collision(rock1)
    player.check_collision(rock2)
    ...
    if player.isDead == False:
        if pygame.key.get_pressed()[K_d] and player.isBlockedRight == False:
            player.lastMove = "D"
            player.rect.x += player.xRightSpeed
        ...
        elif pygame.key.get_pressed()[K_a] and player.isBlockedLeft == False:
            player.lastMove = "A"
            player.rect.x -= player.xLeftSpeed
            player.isIdleDown = False
            player.isWalkingDown = False
            player.isWalkingUp = False
            player.isIdleUp = False
            ...
            player.isIdleLeft = False
        elif pygame.key.get_pressed()[K_w] and player.isBlockedUp == False:
            player.lastMove = "W"
            player.rect.y -= player.yUpSpeed
            player.isIdleDown = False
            player.isWalkingDown = False
            ...
            player.isWalkingLeft = False
            player.isIdleLeft = False
        elif pygame.key.get_pressed()[K_s] and player.isBlockedDown == False:
            player.lastMove = "S"
            player.rect.y += player.yDownSpeed
            player.isIdleDown = False
    ...
    if amountToReset > 4 and player.isImune == True:
        if player.isBlockedRight == False:
            player.xRightSpeed = 8.5
        if player.isBlockedLeft == False:
            player.xLeftSpeed = 8.5
        if player.isBlockedUp == False:
            player.yUpSpeed = 8.5
        if player.isBlockedDown == False:
            player.yDownSpeed = 8.5
        player.isImune = False
        poisonDamageConstant = 2
        normalDamageConstant = 1
